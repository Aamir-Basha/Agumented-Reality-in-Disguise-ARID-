            ...
            async with session.post(url, data=data) as response:
                if response.status == 200:
                    ...
                    if 'application/json' in content_type:
                        ...
                        full_msg = f'{plain_hidden}.     Monokel-ID: {mon_id}'
                        return full_msg
...
async def process_response_monocle(qr_text, response, url, mon_id):
    try:
        if "No hidden text found" in response:
            ...
            print('Nachricht für dich:', plain)  # Print the response from the server if the hidden-text was already stored
    except aiohttp.ClientError as e:
   ...
   try:
    if "No hidden text found" in response:
        ...
        if add_to_db == 'yes':
            ...
            if 'successfully added the data.' in response:
                print('Data was added successfully.\n')
                await display("Data was added successfully.")
            else:
                print('Error while adding new Data')
        ...
        #plain = await decrypt_hidden(response)
        await display(qr_text)
        time.sleep(1)
        await display(response)
        print(f'plain text: {qr_text}')
